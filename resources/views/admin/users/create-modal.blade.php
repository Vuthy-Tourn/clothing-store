...
<form id="createUserForm" method="POST" action="{{ route('admin.users.store') }}" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <!-- Phone -->
        <div>
            <label for="create_phone" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.phone') }}</label>
            <div class="relative">
        ...
        <!-- Date of Birth -->
        <div>
            <label for="create_dob" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.date_of_birth') }}</label>
            <div class="relative">
        ...
        <!-- Gender -->
        <div>
            <label for="create_gender" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.gender') }}</label>
            <div class="relative">
                ...
                <select id="create_gender" name="gender"
                    ...
                    <option value="male" {{ old('gender') == 'male' ? 'selected' : '' }}>{{ __('admin.users.modal.gender_male') }}</option>
                    <option value="female" {{ old('gender') == 'female' ? 'selected' : '' }}>{{ __('admin.users.modal.gender_female') }}</option>
                    <option value="other" {{ old('gender') == 'other' ? 'selected' : '' }}>{{ __('admin.users.modal.gender_other') }}</option>
                </select>
            </div>
        </div>
        ...
        <!-- Address Type -->
        <div>
            <label for="create_address_type" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.address_type') }}</label>
            <div class="relative">
                ...
                <select id="create_address_type" name="address_type"
                        class="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-Ocean focus:border-transparent transition-all duration-300">
                    <option value="shipping" {{ old('address_type') == 'shipping' ? 'selected' : 'selected' }}>{{ __('admin.users.modal.address_type_shipping') }}</option>
                    <option value="billing" {{ old('address_type') == 'billing' ? 'selected' : '' }}>{{ __('admin.users.modal.address_type_billing') }}</option>
                </select>
            </div>
        </div>
        ...
        <!-- Address Name -->
        <div>
            <label for="create_address_name" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.address_name') }}</label>
            <div class="relative">
        ...
        <!-- Address Line 1 -->
        <div class="md:col-span-2">
            <label for="create_address_line1" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.address_line1') }}</label>
            <div class="relative">
        ...
        <!-- Address Line 2 -->
        <div class="md:col-span-2">
            <label for="create_address_line2" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.address_line2') }}</label>
            <div class="relative">
                <i class="fas fa-map-marker-alt absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
        ...
        <!-- City -->
        <div>
            <label for="create_city" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.city') }}</label>
            <div class="relative">
        ...
        <!-- State -->
        <div>
            <label for="create_state" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.state') }}</label>
            <div class="relative">
        ...
        <!-- ZIP Code -->
        <div>
            <label for="create_zip_code" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.zip_code') }}</label>
            <div class="relative">
        ...
        <!-- Country -->
        <div>
            <label for="create_country" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.users.modal.country') }}</label>
            <div class="relative">
        ...
        <div class="md:col-span-2">
            ...
            <div id="createProfilePicturePreview" class="mt-4 hidden">
                ...
                <div class="w-32 h-32 rounded-xl overflow-hidden border-2 border-Ocean">
                    <img id="createProfilePreviewImage" class="w-full h-full object-cover" alt="{{ __('admin.users.modal.preview') }}">
                </div>
            </div>
        </div>
...
document.getElementById('createUserForm').addEventListener('submit', function(e) {
    ...
    // Show loading state
    submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>{{ __("admin.users.messages.creating") }}';
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(data.message || '{{ __("admin.users.messages.success_created") }}');
