<div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200/50">
    ...
    <div class="relative h-32 bg-gradient-to-r from-Ocean to-Ocean/70">
        <div class="absolute -bottom-12 left-1/2 transform -translate-x-1/2">
            <!-- Profile Picture -->
            <div class="relative">
                <div class="w-32 h-32 rounded-2xl border-4 border-white shadow-lg overflow-hidden bg-gradient-to-br from-gray-200 to-gray-300">
                ...
                <div class="absolute bottom-3 right-3">
                    @if ($user->is_active)
                        <span class="w-6 h-6 bg-green-500 rounded-full border-3 border-white shadow-lg animate-pulse"
                            title="{{ __('admin.profile.profile_card.active') }}"></span>
                    @else
                        <span class="w-6 h-6 bg-red-500 rounded-full border-3 border-white shadow-lg"
                            title="{{ __('admin.profile.profile_card.inactive') }}"></span>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="pt-16 pb-6 px-6">
        <div class="text-center mb-6">
            ...
            <div class="inline-flex items-center px-3 py-1 mt-2 rounded-full bg-Ocean/10 text-Ocean text-sm font-medium">
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="text-center p-3 bg-gray-50 rounded-xl">
                ...
                <div class="text-xs text-gray-500">{{ __('admin.profile.profile_card.days_active') }}</div>
            </div>
            <div class="text-center p-3 bg-gray-50 rounded-xl">
                <div class="text-2xl font-bold text-gray-900">
                    {{ $user->loyalty_points ?? 0 }}
                </div>
                <div class="text-xs text-gray-500">{{ __('admin.profile.profile_card.points') }}</div>
            </div>
        </div>
        ...
        <div class="mb-6">
            <div class="flex justify-between items-center mb-2">
                <span class="text-sm font-medium text-gray-900">{{ __('admin.profile.profile_card.profile_complete') }}</span>
            ...
            <p class="text-xs text-gray-500 mt-2">
                {{ __('admin.profile.profile_card.fields_completed', [
...
<div class="mt-6 bg-white rounded-2xl shadow-xl p-6 border border-gray-200/50">
    <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center">
        ...
        {{ __('admin.profile.profile_card.account_stats') }}
    </h3>
    <div class="space-y-3">
        <div class="flex justify-between items-center">
            <span class="text-gray-600">{{ __('admin.profile.profile_card.user_role') }}</span>
            <span class="font-semibold">{{ $stats['account_type'] }}</span>
        </div>
        <div class="flex justify-between items-center">
            <span class="text-gray-600">{{ __('admin.profile.profile_card.status') }}</span>
            <span class="font-semibold {{ $user->is_active ? 'text-green-600' : 'text-red-600' }}">
                {{ $stats['status'] }}
            </span>
        </div>
        <div class="flex justify-between items-center">
            <span class="text-gray-600">{{ __('admin.profile.profile_card.verified') }}</span>
            <span class="font-semibold {{ $user->is_verified ? 'text-green-600' : 'text-yellow-600' }}">
                {{ $stats['verification'] }}
            </span>
        </div>
        <div class="flex justify-between items-center">
            <span class="text-gray-600">{{ __('admin.profile.profile_card.email_verified') }}</span>
