...
@section('content')
    ...
    <section class="mt-8" data-aos="fade-down">
            ...
            <div class="flex items-center gap-4">
                <div class="hidden md:flex items-center space-x-6">
                    ...
                    <div class="text-center">
                        ...
                        <div class="text-2xl font-bold text-gray-900">{{ $arrivals->where('status', 'active')->count() }}</div>
                    </div>
                </div>
                <button onclick="NewArrivalModal.openAdd()"
                    ...
                    <i class="fas fa-plus-circle mr-2 group-hover:rotate-90 transition-transform duration-300"></i> Add New Arrival
                </button>
            </div>
        </div>
        ...
        @if ($arrivals->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" data-aos="fade-up"
                ...
                @foreach ($arrivals as $arrival)
                    <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300 group hover:-translate-y-1"
                        ...
                        <div class="relative bg-gradient-to-br from-gray-50 to-gray-100">
                            ...
                            <div class="absolute top-4 right-4 z-10">
                                <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold shadow-lg {{ $arrival->status === 'active' ? 'bg-gray-900 text-white' : 'bg-gray-700 text-white' }}">
                                    <i class="fas fa-circle text-[6px] mr-1.5 {{ $arrival->status === 'active' ? 'animate-pulse' : '' }}"></i>
                            ...
                            <div
                                ...
                                <div class="flex space-x-2 transform translate-y-6 group-hover:translate-y-0 transition-transform duration-300">
                                    <button onclick="NewArrivalModal.openEdit('{{ $arrival->id }}')"
                                        class="bg-white text-gray-900 p-3 rounded-xl shadow-xl hover:shadow-2xl hover:scale-110 transition-all duration-200 hover:bg-gray-50 group/btn"
                                        title="Edit Product">
                                        <i class="fas fa-edit text-sm group-hover/btn:rotate-12 transition-transform duration-300"></i>
                                    </button>
                                    <button onclick="confirmDelete({{ $arrival->id }}, '{{ addslashes($arrival->name) }}')"
                                        class="bg-white text-gray-900 p-3 rounded-xl shadow-xl hover:shadow-2xl hover:scale-110 transition-all duration-200 hover:bg-gray-900 hover:text-white group/btn"
                                        title="Delete Product">
                                        <i class="fas fa-trash text-sm group-hover/btn:shake transition-transform duration-300"></i>
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Product Info -->
                        <div class="p-5">
                            <h3 class="text-lg font-bold text-gray-900 mb-2 line-clamp-1 group-hover:text-gray-700 transition-colors">
                                {{ $arrival->name }}
                            </h3>
        ...
        @else
            ...
            <div class="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-16 text-center transition-all hover:border-gray-300"
                ...
                <div class="w-24 h-24 bg-gray-50 rounded-2xl flex items-center justify-center mx-auto mb-6 border-4 border-gray-100">
    ...
    <div id="addNewArrivalModal"
        ...
        <div class="bg-white rounded-3xl w-full max-w-4xl shadow-2xl border border-gray-200">
            <!-- Modal Header -->
            ...
            <div class="overflow-y-auto" style="max-height: calc(90vh - 180px);">
                <form action="{{ route('admin.new-arrivals.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <span class="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-700 font-medium">$</span>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <label class="relative cursor-pointer">
                                        <input type="radio" name="status" id="edit_status_active" value="active" class="sr-only peer">
                                        <div class="p-4 border-2 border-gray-200 rounded-xl text-center peer-checked:border-gray-900 peer-checked:bg-gray-50 transition-all duration-200 hover:bg-gray-50">
                                            <i class="fas fa-check-circle text-gray-700 mb-2"></i>
                                            <p class="font-medium text-gray-900">Active</p>
                                            <p class="text-gray-700 text-sm">Visible to customers</p>
                                    ...
                                    <label class="relative cursor-pointer">
                                        <input type="radio" name="status" id="edit_status_inactive" value="inactive" class="sr-only peer">
                                        <div class="p-4 border-2 border-gray-200 rounded-xl text-center peer-checked:border-gray-900 peer-checked:bg-gray-50 transition-all duration-200 hover:bg-gray-50">
                                            <i class="fas fa-eye-slash text-gray-700 mb-2"></i>
                                            <p class="font-medium text-gray-900">Inactive</p>
                                            <p class="text-gray-700 text-sm">Hidden from customers</p>
