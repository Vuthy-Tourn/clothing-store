...
@section('content')
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <div class="flex items-center">
                                <div
                                    class="w-3 h-3 rounded-full {{ $product->is_active ? 'bg-green-500' : 'bg-red-500' }} mr-2">
                                </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-4">
                                <div class="text-center">
                                    ...
                                    <span
                                        class="text-Wave line-through">${{ number_format($product->original_price) }}</span>
                                </div>
                                <div class="text-center">
                                    <span class="block text-Wave text-sm">Discounted</span>
                                    <span
                                        class="text-2xl font-bold text-Ocean">${{ number_format($product->discounted_price) }}</span>
                                </div>
                            </div>
            ...
            <div class="space-y-6" data-aos="fade-left">
                ...
                <div class="bg-Pearl border border-Silk rounded-xl p-6 shadow-sm">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="#"
                            class="block bg-Lace text-Ocean hover:bg-Silk py-3 rounded-lg font-medium text-center transition-colors">
    ...
    @else
        ...
        <div class="flex flex-col items-center justify-center py-12" data-aos="fade-up">
            ...
            <p class="text-white text-lg mb-2 text-center max-w-md">Highlight your best product to drive sales and
                engagement</p>
    ...
    @if ($product)
        <!-- Edit Modal -->
        <div id="editFeaturedModal"
            class="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
            <div
                class="bg-Pearl w-full max-w-2xl rounded-xl shadow-lg p-6 relative border border-Silk max-h-[90vh] overflow-y-auto">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-xl font-bold text-Ocean  ">Edit Featured Product</h2>
                    <button onclick="FeaturedModal.closeEdit()"
                        class="text-Wave hover:text-Ocean text-xl transition-colors">
                        <i class="fas fa-times"></i>
                    </button>
                </div>

                <form method="POST" action="{{ route('admin.featured.update', $product->id) }}"
    ...
    <div id="featuredModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
        <div
            ...
            <form id="featuredProductForm" method="POST" enctype="multipart/form-data"
                action="{{ route('admin.featured.store') }}">
                @csrf
