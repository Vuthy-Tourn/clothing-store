            ...
            <div class="md:flex md:items-center md:justify-between">
                <div>
                    ...
                    <p class="mt-2 text-gray-600">
                        @if($products->count() > 0)
                            Found {{ $products->total() }} result{{ $products->total() > 1 ? 's' : '' }} for "<span class="font-semibold text-gray-900">{{ $searchQuery }}</span>"
                        @else
        ...
        @if($products->count() > 0)
            ...
            <div class="mb-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <select id="sortSelect" class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-gray-900 focus:border-transparent">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach($products as $product)
                    ...
                    <div class="group bg-white rounded-lg overflow-hidden border border-gray-200 hover:border-gray-300 transition-all duration-200 hover:shadow-lg">
                        <!-- Product Image -->
                        <a href="{{ route('product.view', $product->slug) }}" class="block relative overflow-hidden bg-gray-100">
                            @if($primaryImage)
                            ...
                            <div class="absolute top-3 left-3 flex flex-col gap-1">
                                @if($product->is_new)
                                    <span class="px-2 py-1 bg-blue-600 text-white text-xs font-semibold rounded">NEW</span>
                                @endif
                                @if($product->is_featured)
                                    <span class="px-2 py-1 bg-purple-600 text-white text-xs font-semibold rounded">FEATURED</span>
                                @endif
                                @if($hasDiscount)
                            ...
                            <div class="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                <button class="p-2 bg-white rounded-full shadow-md hover:bg-gray-50 transition-colors"
                        ...
                        <div class="p-4">
                            ...
                            {{-- @if($product->category)
                                <a href="{{ route('products.category', $product->category->slug) }}" 
                                   class="text-xs font-medium text-gray-500 hover:text-gray-700 uppercase tracking-wide">
                            ...
                            <a href="{{ route('product.view', $product->slug) }}">
                                <h3 class="mt-1 text-sm font-semibold text-gray-900 group-hover:text-gray-700 line-clamp-2">
                                    {{ $product->name }}
                                </h3>
                            </a>
                            ...
                            <div class="mt-3 flex items-center justify-between">
                                <div class="flex items-center space-x-2">
                                    @if($hasDiscount && $discountPrice)
                                        <span class="text-lg font-bold text-gray-900">${{ number_format($discountPrice, 2) }}</span>
                                        <span class="text-sm text-gray-500 line-through">${{ number_format($minPrice, 2) }}</span>
                                    @elseif($minPrice == $maxPrice)
                                        <span class="text-lg font-bold text-gray-900">${{ number_format($minPrice, 2) }}</span>
                                    @else
                                        <span class="text-lg font-bold text-gray-900">${{ number_format($minPrice, 2) }} - ${{ number_format($maxPrice, 2) }}</span>
                                    @endif
                                </div>
                                
                                <!-- Add to Cart Button -->
                                <button onclick="addToCartFromSearch('{{ $product->slug }}')"
                                        class="px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-800 transition-colors opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-200">
